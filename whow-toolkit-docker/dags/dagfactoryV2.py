...
async def communicate_with(service_uri: str, message = ''):
    ...
    async with websockets.connect(service_uri, ping_interval=None) as websocket:
...
#g.parse({{ graph_uri }}', format='ttl')
g.parse('http://toolkit:5000/dagstore/api/dags', format='ttl')
